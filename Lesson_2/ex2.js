const data = [
  ...
  function averageAge(data) {
    ...
    averageAge = sum / data.length;
    return averageAge
  }
  
  function moreThen25(data) {
    const namesOfMoreThan25 = new Array();
    for(i = 0; i < data.length; i++) {
      if(data[i].age > 25) {
        namesOfMoreThan25.push(data[i].firstname)
      }
    }
    return namesOfMoreThan25;
  }
  
  function emailCom(data) {
    const emailFinishedCom = new Array();
    for(i = 0; i < data.length; i++) {
      if(data[i].email.endsWith('.com')) {
        emailFinishedCom.push(data[i].lastname)
      }
    }
    return emailFinishedCom;
}
  
  function sexCount(data) {
    let maleCounter = 0;
    let femaleCounter = 0;
    ...
    const sexCountData = new Array();
    sexCountData.push("Female : " + femaleCounter, "Male : " + maleCounter);
    return sexCountData;
  }
